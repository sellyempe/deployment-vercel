...
<head>
    ...
    .flatpickr-day.enabled-date::after {
        content: "";
        position: absolute;
        bottom: 4px;
        left: 50%;
...
<body class="font-poppins bg-slate-50 text-slate-900">
        ...
        <section class="pt-32 pb-24 px-4 min-h-screen">
            <div class="max-w-5xl mx-auto">
                <a href="{{ route('trip.detail', $trip->id) }}"
                    class="text-pink-600 hover:text-pink-700 font-semibold mb-6 inline-flex items-center gap-2 relative z-10">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
                    <div class="lg:col-span-2">
                        <div
                            class="bg-white rounded-[2.5rem] p-8 md:p-12 border border-slate-100 shadow-sm relative overflow-hidden">
                            <div
                            ...
                            <form action="{{ route('booking.store') }}" method="POST" class="space-y-8 relative z-10">
                                ...
                                <div class="bg-slate-50 rounded-3xl p-6 border border-slate-100 space-y-4">
                                    ...
                                    <div class="flex justify-between items-center text-sm">
                                        ...
                                        <span class="font-bold text-pink-600">Rp
                                            {{ number_format($trip->price, 0, ',', '.') }}</span>
                                    </div>
                                    <div class="pt-3 border-t border-slate-200 flex justify-between items-center">
                                        <span class="text-slate-500 font-medium">Sisa Kuota (<span id="quota-date-label"
                                                class="text-slate-400">Pilih Tanggal</span>)</span>
                                        <span class="font-bold text-slate-900"><span
                                                id="available-seats-display">--</span> kursi</span>
                                    </div>
                                </div>
                                ...
                                <div class="space-y-3">
                                    ...
                                    <input type="tel" name="phone" value="{{ old('phone') }}" placeholder="0812..."
                                        ...
                                        oninvalid="this.setCustomValidity(this.validity.valueMissing ? 'Harap isi bagian ini.' : 'Nomor telepon hanya boleh berisi angka (10-13 digit).')"
                                ...
                                <div class="space-y-3">
                                    <label class="block text-sm font-bold text-slate-700">Catatan Khusus
                                        (Opsional)</label>
                                ...
                                <div class="bg-pink-600 rounded-3xl p-8 text-white shadow-xl shadow-pink-600/20">
                                    <div class="flex justify-between items-center">
                                        <div>
                                            ...
                                            <p id="subtotal" class="text-3xl font-black">Rp
                                                {{ number_format($trip->price, 0, ',', '.') }}</p>
                                        </div>
                                        <button type="submit" id="submit-btn"
                                            class="bg-white text-pink-600 px-8 py-4 rounded-2xl font-bold hover:bg-pink-50 transition-all shadow-lg active:scale-95 disabled:opacity-50 disabled:scale-100">
                                            Booking
                                        </button>
                                    </div>
                                </div>
                                ...
                                <p class="text-center text-xs text-slate-400">
                                    Dengan mengklik Booking, Anda setuju dengan <a href="{{ route('terms') }}"
                                        class="underline hover:text-pink-600">Syarat & Ketentuan</a> kami.
                                </p>
                            </form>
                        </div>
                    </div>
                    ...
                    <div class="lg:col-span-1">
                        <div
                            class="bg-white rounded-[2.5rem] p-6 border border-slate-100 shadow-xl shadow-slate-200/50 sticky top-28 space-y-6">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        async function checkAvailability(selectedDate) {
            ...
            quotaDateLabel.textContent = new Date(selectedDate).toLocaleDateString('id-ID', {
                day: 'numeric',
                month: 'short'
            });
            ...
            try {
                const response = await fetch(
                    `${checkAvailabilityUrl}?trip_id=${tripId}&date=${selectedDate}`);
